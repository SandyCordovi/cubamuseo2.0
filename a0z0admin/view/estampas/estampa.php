...
<div class="cui_row admin-contenedor">
    <h1 class="admin-titulo">ESTAMPAS</h1>
    <div class="cui_row" style="margin-top: 20px;">
        <div class="btn_admin" onclick="cui_wnd._createWND_tamFijo('wnd/estampas/estampa_nuevo.php', 70, function(){CreateEditor();});">
            Nuevo
        </div>
        <div class="btn_admin" onclick="Refresh();">
            Refresh
        </div>
        <form id="ajaxform" action="service/estampas/estampa.php" method="post" class="admin_search_box">
            <input id="search_admin" name="b" />
            <input type="submit" value=""/>
            <input type="hidden" name="p" value="1" />
            <input id="opt_h" type="hidden" name="opt" value="8" />
        </form>
    </div>
     <div class="cui_row" style="margin-top: 50px;">
        <select id="categoria_select" class="form" style="width: 500px; margin-bottom: 0;">
...
$(document).ready(function() {    
    ...
    $.ajax(
    {
         url : 'service/estampas/estampa.php',
        type: "POST",
        data : "p=1&b="+$('#search_admin').val()+"&opt="+$('#categoria_select').val(),
        datatype: "html",
        success:function(data, textStatus, jqXHR)
        {
            $('.cui_result_table').html(data);
...
function Edit(id)
{
    cui_wnd._createWND_tamFijo('wnd/estampas/estampa_edit.php?id='+id, 70, function(){CreateEditor();});
}
...
{
    $.ajax({
        url : 'service/estampas/estampa_cmd.php',
        type: "POST",
        data : 'cmd=3&id='+id,
        success:function(data, textStatus, jqXHR)
        {
